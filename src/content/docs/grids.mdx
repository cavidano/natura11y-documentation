...
You can control which axis (*x* or *y*) uses continuous lines. Chain: `--fill-x` or `--fill-y` modifiers to `.grid--divider` selectors. With `.grid--divider--fill-x` present on the example below, the horizontal lines connect.
